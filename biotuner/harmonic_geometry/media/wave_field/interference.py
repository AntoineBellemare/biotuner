...
Belongs to the ``wave_field`` family of :mod:`biotuner.harmonic_geometry.media`:
the operator here is a coherent superposition of travelling / standing
plane waves in free space, with no boundary eigenproblem. Five rendering
paradigms are exposed as functions; the class wrapper :class:`Interference`
plugs into the pipeline contract defined in
:mod:`biotuner.harmonic_geometry.media.base`.
...
_INTERFERENCE_PARADIGMS = {
    "harmonic": harmonic_interference_field_2d,
    "quasicrystal": quasicrystal_field_2d,
    "standing_lattice": standing_wave_lattice_2d,
    "vortex": vortex_field_2d,
    "sources": interference_field_2d,
}


class Interference(_Medium):
    """Wave-field-family medium: open-medium coherent superposition.

    Wraps the five paradigm functions in this module behind the pipeline
    contract defined by
    :class:`biotuner.harmonic_geometry.media.base.Medium`. The paradigm
    is selected at construction; per-call kwargs are forwarded.

    Parameters
    ----------
    paradigm : str, default='harmonic'
        One of ``'harmonic'`` (rotational averaging),
        ``'quasicrystal'`` (N-fold quasi-periodic),
        ``'standing_lattice'`` (Cartesian 2-D lattice),
        ``'vortex'`` (spiral / topological),
        ``'sources'`` (N-source Young's-style fringes).
    **defaults
        Default kwargs for the chosen paradigm (e.g. ``n_directions``,
        ``extent``, ``resolution``). Per-call overrides take precedence.
    """

    family = "wave_field"

    def __init__(self, *, paradigm: str = "harmonic", **defaults) -> None:
        if paradigm not in _INTERFERENCE_PARADIGMS:
            raise ValueError(
                f"paradigm must be one of {sorted(_INTERFERENCE_PARADIGMS)}; "
                f"got {paradigm!r}."
            )
    ...
    def respond(
        ...
        **overrides,
        ...
        if not isinstance(forcing, HarmonicInput):
            raise TypeError(
                "Interference.respond requires a HarmonicInput; got "
                f"{type(forcing).__name__}."
            )
        kwargs = {**self.defaults, **overrides}
        return _INTERFERENCE_PARADIGMS[self.paradigm](forcing, **kwargs)
    ...
    def __repr__(self) -> str:
        return f"Interference(paradigm={self.paradigm!r})"
