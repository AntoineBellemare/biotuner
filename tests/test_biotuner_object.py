...
def create_test_signal(sampling_frequency, duration, peak_frequencies, amplitudes=None, noise_level=0.0):
...
@pytest.fixture
def test_signal():
    return create_test_signal(SAMPLING_FREQUENCY, DURATION, PEAK_FREQUENCIES, AMPLITUDES, NOISE_LEVEL)
...
def test_peaks_extraction(biotuner_instance, test_signal):
    ...
    for freq in PEAK_FREQUENCIES:
        assert any(abs(freq - detected) < 1 for detected in bt.peaks), f"Peak {freq} not found"
...
def test_known_output(biotuner_instance):
    bt = biotuner_instance
    test_signal = create_test_signal(SAMPLING_FREQUENCY, DURATION, PEAK_FREQUENCIES, AMPLITUDES, NOISE_LEVEL)
    bt.peaks_extraction(test_signal, precision=1)
