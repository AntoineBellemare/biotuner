...
import type { Chord } from "../reels/cymatics";
...
export const MultiGeometryMorph: React.FC<{ data: ReelData }> = ({ data }) => {
  ...
  // Fade the geometry in/out within each segment so transitions between
  // different geometry types are clean.
  ...
  // Cymatics gamma bloom (only used for the cymatics kind).
  const gamma = 0.72 + 0.4 * Math.sin(Math.PI * local);
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      if (ctx) {
        if (kind === "cymatics") drawCymatics(ctx, chord.ratios, gamma);
        else if (kind === "lissajous")
          drawLissajous(ctx, chord.ratios, frame % seg, fps);
        else if (kind === "harmonograph")
          drawHarmonograph(ctx, chord.ratios, frame % seg, fps);
        else if (kind === "interference")
          drawInterference(ctx, chord.ratios, frame, fps);
      }
    }
